...
class Circle(Shape):
    def __init__(self, x, y, radius, fill_color, border_color, border_width, zindex, isclicked):
        super().__init__(x, y, fill_color, border_color, border_width, zindex, isclicked)
        self.radius = radius
...
class MyPaintArea(QWidget):
    ...
    def circle(self, x,y, radius, fill_color, border_color, border_width, zindex, isclicked):
        self.scene.add_object(Circle(x, y, radius, fill_color, border_color, border_width, zindex,isclicked))
        self.update()
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if self.selected_object:
                ...
                self.zoom_object = self.selected_object
                self.update()
            else:
        ...
        if event.button() == Qt.RightButton:
            test = self.find_object_at(event.position())
            if self.last_object == test:
                if self.last_object.isclicked == True:
                    self.zoom_object = self.last_object
                    self.update()
                else:
                    self.zoom_object = None
                    self.slastobject.isclicked = False
                    self.last_object = None
                    self.update()
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.zoom_object:
            dw = event.position().x() - self.last_mouse_position.x()
            dh = event.position().y() - self.last_mouse_position.y()
            self.last_mouse_position = event.position()
            

            if isinstance(self.zoom_object,(Rectangle)):
                self.zoom_object.width = self.zoom_object.width + dw
                self.zoom_object.height = self.zoom_object.height + dh

            if isinstance(self.zoom_object,(Circle)):
                self.zoom_object.radius = self.zoom_object.radius +(dw/2)
        self.update()
